  ...
  const onFinish=async(values)=>{
    try {
      ...
      if(selectedItemForEdit){
          response = await axios.post("/api/portfolio/update-miniprojects", {
          ...values,
          _id: selectedItemForEdit._id,
        });
      }
      else{
        response = await axios.post("/api/portfolio/add-miniprojects", 
        values,
);
      }
...
const onDelete=async(item)=>{
  try {
    ...
    dispatch(ShowLoading());
    dispatch(reloadData(true))
        const response = await axios.post("/api/portfolio/delete-miniprojects", {
        _id: item._id,
      });
...
export default AdminMiniProjects;
